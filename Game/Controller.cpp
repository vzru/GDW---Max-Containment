...
namespace Input
{
	...
	bool XBoxInput::Update(unsigned short _numControllers) {
		// download packets
		unsigned int result;
...
#pragma region Update Sticks
					...
					if (glm::length(m_sticks[i].first) > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE)
						...
						m_sticks[i].first = { 0.0f, 0.0f };


					if (m_sticks[i].second.length() > XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE)
						m_sticks[i].second /= 32767.0f;
					else
						m_sticks[i].second = { 0.0f, 0.0f };
#pragma endregion
...
#pragma endregion
				...
				Triggers triggers = GetTriggers(i);
				Sticks sticks = GetSticks(i);
				for (int b = 0; b < 14; b++)
					if (GetButton(i, (Input::Button)b))
						(*callback)(i, (Input::Button)b);
				(*special)(i, triggers, sticks);
			} else
				m_connected[i] = false;
		}
	...
	Sticks XBoxInput::GetSticks(unsigned short _index) {
		if (!m_connected[_index]) {/*error here*/}
		return m_sticks[_index];
	}

	bool XBoxInput::GetConnected(unsigned short _index) {
		if (!m_connected[_index]) return false;
		return true;
	}

	bool XBoxInput::GetButton(unsigned short _index, Button _buttons) {
		if (!m_connected[_index]) {/*error here*/ }
		return m_buttonStates[_index][_buttons];
	}

	Triggers XBoxInput::GetTriggers(unsigned short _index) {
		if (!m_connected[_index]) {/*error here*/}
		return m_triggers[_index];
	}

	bool XBoxInput::SetVibration(unsigned short _index, Triggers _powers) {
		if (!m_connected[_index]) {/*error here*/ }
		XINPUT_VIBRATION vibration;
		memset(&vibration, 0, sizeof(XINPUT_VIBRATION));
	...
	void XBoxInput::setCallbacks(void(*_callback)(unsigned short index, Input::Button button), void(*_special)(unsigned short index, Input::Triggers triggers, Input::Sticks sticks)) {
		callback = _callback;
		special = _special;
	}
}