...
void Game::init(void(*_controllerInput)(unsigned short index, Input::Button button), void(*_controllerSpecial)(unsigned short index, Input::Triggers triggers, Input::Sticks sticks)) {
	...
	screen.quad = new Object();
	screen.quad->loadMesh(assets->meshes["screen"]);
	program["Unlit"] = new Shader();
	if (!program["Unlit"]->load("assets/shaders/animation.vert", "assets/shaders/unlitTexture_f.glsl")) {
		std::cout << "Unlit shader failed to initialize." << std::endl;
		system("pause");
		exit(0);
	}
	...
	screen.quad->draw(program["Unlit"], screen.camera, { }, 0);
	...
	if (!program["NoFlashNoNorm"]->load("assets/shaders/animation.vert", "assets/shaders/NoFlashNoNorm.frag")) {
		std::cout << "noflashnonorm Shaders failed to initialize." << std::endl;
		system("pause");
		exit(0);
	}
	program["MeshMorph"] = new Shader();
	if (!program["MeshMorph"]->load("assets/shaders/animation.vert", "assets/shaders/phongSpot.frag")) {
		std::cout << "meshmorph Shaders failed to initialize." << std::endl;
		system("pause");
		exit(0);
	}
	...
	if (!program["particles"]->load("assets/shaders/passThru.glsl", "assets/shaders/particles_g.glsl", "assets/shaders/unlitTexture_f.glsl")) {
		std::cout << "particles Shaders failed to initialize." << std::endl;
		system("pause");
		exit(0);
	}
	...
	std::cout << glutGet(GLUT_ELAPSED_TIME) << " milliseconds to load in everything" << std::endl;
...
void Game::initializeParticles()
{
	...
	part->lifeR = { 1.0f, 5.0f, 0.0f };
	part->initForceMin = { -0.1f, 0.0f, -0.1f };
	part->initForceMax = { 0.1f, 0.1f, 0.1f };
...
void Game::draw() {
	...
	case State::Pause:
		...
		screen.pause->draw(program["Unlit"], screen.camera, {  }, 0);
		break;
	case State::Play:
		if (lightOn)
		{
